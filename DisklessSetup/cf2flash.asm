;
; Carnivore2+ CF2Flash Utility v1.11
; Created by Vladimir & RBSC [2022-2024]
;
...
;Program 64kb Boot block + directory
	...
	ld	a,4			; 4th boot menu block
	ld	(PreBnk),a
	ld	hl,#80
	ld	de,#8000
	call	FBProgCF		; 8kb block 4
	jp	c,FlashFailed
	ld	hl,#90
	ld	de,#A000
	call	FBProgCF		; 8kb block 4
	...
	call	SET2PD
	print	SDWrit			; Writing SD BIOS
	ld	a,#04
	ld	(Record+2),a		; set start block #40000
	ld	a,#02
	ld	(Record+3),a		; set length 2 bl #40000-#5FFFF
	ld	(Size+2),a
	ld	hl,0
	ld	(Size),hl
	ld	hl,#80	
...
SDWrit:
	db	13,10
	db	"Writing SD BIOS into FlashROM...",13,10,"$"
FmpacWrit:
	db	13,10
...
SLOTN:	
	db	13,10
	db	"Carnivore2+ CF2Flash Utility v1.11",13,10
	db	"Created by Vladimir & RBSC [2022-2024]",13,10,13,10
	db	"Enter Carnivore2+ slot number: $"
SUBSLOTN:
